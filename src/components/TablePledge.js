...
const TablePledge = ({ data }) => {
  const { name, earlyPrice, regularPrice, latePrice } = data.tablesYaml;
  let price = earlyPrice;
  const now = Date.now();
    ...
    <Layout>
      <StyledForm success={success}>
        <form onSubmit={e => submitForm(e)} required>
          <Heading>{`I Want to Sponsor a ${name} Table`}</Heading>
...
TablePledge.propTypes = {
  data: PropTypes.object,
};

export default TablePledge;

export const query = graphql`
  query($name: String!) {
    tablesYaml(name: { eq: $name }) {
      name
      earlyPrice
      regularPrice
      latePrice
